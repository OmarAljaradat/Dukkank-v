...
- DO NOT return Markdown wrapping or extra text. Only raw JSON array!`;
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-slate-900 text-white p-5 sm:p-6 rounded-3xl border border-slate-800 shadow-xl flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-2 shrink-0">
                    <button
                        ...
                        className="px-5 py-3 rounded-2xl bg-gradient-to-r from-purple-600 via-blue-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white text-xs font-black flex items-center gap-2 shadow-lg transition-all"
                    >
                        <Bot className="w-4.5 h-4.5 animate-bounce" />
                        <span>🤖 مستورد الذكاء الاصطناعي لكشوفات الموردين</span>
                    </button>
            ...
            {/* Robust AI Supplier Auto-Importer Modal */}
            {aiModalOpen && (
                <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 overflow-y-auto">
                    <div className="bg-slate-900 text-white rounded-3xl border border-slate-800 max-w-4xl w-full p-6 sm:p-8 space-y-6 shadow-2xl animate-in fade-in zoom-in-95 my-8 text-right" dir="rtl">
                        {/* AI Modal Header */}
                        <div className="flex items-center justify-between border-b border-slate-800 pb-4">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-slate-400 font-medium">تحليل كشوفات الألعاب بالواتساب أو الملفات، جلب الصور تلقائياً وحساب الأسعار بالمعادلة الذكية.</p>
                                </div>
                            </div>

                            <button onClick={() => setAiModalOpen(false)} className="p-2 rounded-xl bg-slate-800 text-slate-400 hover:text-white">
                                <X className="w-5 h-5" />
                            </button>
                        </div>

                        {/* Step 1: Inputs & Pricing Formula Settings */}
                        <div className="space-y-4">
                            ...
                            <div className="p-4 rounded-2xl bg-purple-950/40 border border-purple-800/50 space-y-2">
                                <div className="flex items-center justify-between flex-wrap gap-2">
                                    <label className="block text-xs font-black text-purple-300">
                                        🔑 أدخل مفتاح Google Gemini API Key الخاص بك (للحصول على تحليل ذكي 100%):
                                    </label>
                                    <a
                                    ...
                                    >
                                        <span>احصل على مفتاح مجاني فوراً من Google AI Studio ↗</span>
                                    </a>
                                </div>
                                <input
                                    type="password"
                                    ...
                                    onChange={(e) => {
                                        ...
                                        localStorage.setItem("dukkank_gemini_key", e.target.value);
                                    }}
                                    placeholder="إلصق المفتاح هنا: AIzaSy..."
                                    className="w-full px-3.5 py-2.5 rounded-xl bg-slate-950 border border-slate-700 text-xs font-mono font-bold text-amber-400 placeholder:text-slate-600 focus:border-purple-500 focus:outline-none"
                                />
                            </div>
                            ...
                            <div className="space-y-1.5">
                                <label className="block text-xs font-bold text-slate-300">
                                    1. إلصق كشف الواتساب أو الألعاب المتوفرة من المورد (مهما كان تنسيقه):
                                </label>
                                <textarea
                                    rows={4}
                                    value={aiRawInput}
                                    onChange={(e) => setAiRawInput(e.target.value)}
                                    placeholder={`مثال لصق الرسالة بجميع الصيغ:\n- FC 25 ps5 بـ 35$\n- قراند 5 = 15 USD\n- Black Ops 6 | 45 دولار\n- Elden Ring : 40`}
                                    className="w-full p-3.5 rounded-2xl bg-slate-950 border border-slate-800 text-xs font-bold text-slate-200 placeholder:text-slate-600 focus:border-purple-500 focus:outline-none"
                                />
                            </div>

                            {/* Image / PDF Uploader Helper */}
                            <div className="p-3.5 rounded-2xl bg-slate-950 border border-slate-800 space-y-3">
                                <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
                                    <div className="flex items-center gap-2 text-xs font-extrabold text-slate-300">
                                        <UploadCloud className="w-4 h-4 text-purple-400" />
                                        <span>أو ارفع صورة كشف المورد مباشرة (Gemini Flash Vision OCR):</span>
                                    </div>
                                    <label className="px-4 py-2 rounded-xl bg-purple-600/20 hover:bg-purple-600/30 text-purple-300 border border-purple-500/30 text-xs font-black cursor-pointer transition flex items-center gap-2">
                                        <span>📷 رفع صورة كشف المورد (سكرينشوت)</span>
                                        <input
                                            type="file"
                                            accept="image/*"
                                            className="hidden"
                                            onChange={(e) => {
                                                ...
                                                if (file) {
                                                    ...
                                                    reader.onload = () => {
                                                        ...
                                                        toast.success(`تم إرفاق صورة الكشف "${file.name}" لقراءتها بواسطة Gemini Vision 📸`);
                                                    };
                                ...
                                {/* Base64 Image Preview Box */}
                                {aiImageBase64 && (
                                    <div className="p-3 rounded-2xl bg-purple-950/40 border border-purple-800/60 flex items-center justify-between gap-3 animate-in fade-in">
                                        <div className="flex items-center gap-3">
                                            <img src={aiImageBase64} alt="كشف المورد" className="w-16 h-16 rounded-xl object-cover border border-purple-500/40 shadow" />
                                            <div>
                                                <span className="block text-xs font-black text-purple-300">📸 صورة الكشف مرفقة ومستعدة للتحليل المباشر</span>
                                                <span className="text-[10px] text-slate-400">سيقوم Gemini 1.5 Flash Vision بكسح الصورة وقراءة أسعار الألعاب من البكسلات.</span>
                                            </div>
                                        </div>
                                        <button
                                            type="button"
                                            onClick={() => setAiImageBase64(null)}
                                            className="px-3 py-1.5 rounded-xl bg-red-950/60 border border-red-800 text-red-400 text-xs font-bold hover:bg-red-900 transition"
                                        >
                                            إزالة الصورة
                                        </button>
                                    </div>
                            ...
                            <div className="p-4 rounded-2xl bg-purple-950/30 border border-purple-800/40 space-y-3">
                                <div className="flex items-center gap-2 text-xs font-black text-purple-300">
                                    ...
                                    <span>2. معادلة الذكاء الاصطناعي لحساب سعر بيعك بالمتجر:</span>
                                </div>

                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div className="space-y-1">
                                        <label className="block text-[11px] font-bold text-slate-300">إضافة نسبة % على سعر المورد:</label>
                                        <div className="flex items-center gap-2">
                                            <input
                                                type="number"
                                                value={aiMarkupPct}
                                                onChange={(e) => setAiMarkupPct(parseFloat(e.target.value) || 0)}
                                                className="w-full px-3 py-2 rounded-xl bg-slate-900 border border-slate-700 text-xs font-black text-purple-300 text-center"
                                            />
                                    ...
                                    <div className="space-y-1">
                                        <label className="block text-[11px] font-bold text-slate-300">إضافة هامش ربح ثابت ($):</label>
                                        <div className="flex items-center gap-2">
                                            <input
                                                type="number"
                                                value={aiFixedProfit}
                                                onChange={(e) => setAiFixedProfit(parseFloat(e.target.value) || 0)}
                                                className="w-full px-3 py-2 rounded-xl bg-slate-900 border border-slate-700 text-xs font-black text-emerald-400 text-center"
                                            />
                                ...
                                <div className="text-[11px] font-extrabold text-slate-400 pt-1">
                                    * المعادلة المطبقة: <strong className="text-white">سعر المتجر = سعر المورد + ({aiMarkupPct}%) + ${aiFixedProfit} هامش صافي ربح لك</strong>.
                                </div>
                            </div>
                            ...
                            <button
                                ...
                                className="w-full py-3.5 rounded-2xl bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white font-black text-xs shadow-lg transition flex items-center justify-center gap-2"
                            >
                                {aiProcessing ? <Loader2 className="w-4 h-4 animate-spin" /> : <Wand2 className="w-4 h-4" />}
                                <span>تحليل الألعاب وجلب الصور وحساب الأسعار بالذكاء الاصطناعي ✨</span>
                            </button>
                        </div>

                        {/* Step 2: Editable Parsed Preview Results & Mass Publish */}
                            ...
                            <div className="space-y-4 pt-4 border-t border-slate-800 animate-in fade-in">
                                <div className="flex items-center justify-between">
                                    <h4 className="text-xs font-black text-emerald-400 flex items-center gap-1.5">
                                        ...
                                        <span>جدول المعاينة والتعديل المباشر قبل النشر ({aiParsedResults.length} لعبة)</span>
                                    </h4>
                                    <span className="text-[11px] text-slate-400">يمكنك تعديل أي خانة بالجدول أدناه قبل الضغط على نشر!</span>
                                </div>

                                <div className="max-h-64 overflow-y-auto space-y-2 pr-1 border border-slate-800 p-2 rounded-2xl bg-slate-950">
                                    {aiParsedResults.map((item, idx) => (
                                        <div key={idx} className="p-3 rounded-2xl bg-slate-900 border border-slate-800 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 text-xs">
                                            <div className="flex items-center gap-3 flex-1">
                                                <img src={item.image} alt={item.name} className="w-10 h-14 rounded-lg object-cover shrink-0 border border-slate-700" />
                                                <div className="space-y-1 flex-1">
                                                    <input
                                                        type="text"
                                                        value={item.name}
                                                        onChange={(e) => handleUpdateParsedItem(idx, "name", e.target.value)}
                                                        className="w-full bg-slate-950 border border-slate-800 px-2 py-1 rounded font-black text-white text-xs focus:border-blue-500"
                                                    />
                                                    <div className="text-[10px] text-slate-400 line-clamp-1">{item.sub}</div>
                                                </div>
                                            </div>

                                            <div className="flex items-center gap-3 justify-between sm:justify-end shrink-0 border-t sm:border-t-0 pt-2 sm:pt-0 border-slate-800">
                                                <div className="space-y-0.5 text-center">
                                                    <span className="text-[10px] text-slate-400 block">سعر المورد</span>
                                                    <span className="text-slate-300 font-bold">${item.supplierCost}</span>
                                                </div>

                                                <div className="space-y-0.5 text-center">
                                                    <span className="text-[10px] text-blue-400 font-bold block">سعر PS5</span>
                                                    <input
                                                        type="number"
                                                        step="0.01"
                                                        value={item.five}
                                                        onChange={(e) => handleUpdateParsedItem(idx, "five", parseFloat(e.target.value) || 0)}
                                                        className="w-16 bg-slate-950 border border-blue-500 text-emerald-400 text-center font-black px-1 py-0.5 rounded text-xs"
                                                    />
                                                </div>

                                                <div className="space-y-0.5 text-center">
                                                    <span className="text-[10px] text-amber-400 font-bold block">ربحك الصافي</span>
                                                    <span className="text-amber-400 font-black">+${(item.five - item.supplierCost).toFixed(2)}</span>
                                                </div>
                                            </div>
                                        </div>
                                    ))}
                                ...
                                <button
                                    ...
                                    disabled={busy}
                                    className="w-full py-4 rounded-2xl bg-emerald-600 hover:bg-emerald-700 text-white font-black text-sm shadow-xl transition flex items-center justify-center gap-2"
                                >
                                    {busy ? <Loader2 className="w-5 h-5 animate-spin" /> : <CheckCircle2 className="w-5 h-5" />}
                                    <span>📥 نشر كافة هذه الألعاب فوراً في أول الصفحة الرئيسية للمتجر</span>
                                </button>
                            </div>
                        )}
                    </div>
